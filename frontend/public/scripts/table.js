import { deleteItem, getItems, filterItems } from "./api.js";
...
function drawTable(items) {
  ...
  for (const item of items) {
    ...
    row.insertCell().innerText = String(item.price);
...
export async function handleFilterItem() {
  ...
  const items = await filterItems(name, Number(lowerPrice), Number(upperPrice));
